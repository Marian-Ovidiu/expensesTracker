            ...
            <div class="col-12 col-md-9 mx-auto">
                ...
                <form action="{{ route('add.expense') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" step="0.01" class="form-control  @error('expense_date') is-invalid @enderror" id="expense_date" name="expense_date"  value="{{ old('expense_date') }}" required>
                        @error('expense_date')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                            ...
                            @if($expense->status === "active")
                                <tr class="text-center">
                                    ...
                                    <form action="{{route('delete.expense', ['expense' => $expense])}}" method="post">
                                        @csrf
                                        @method('POST')
                                        <td class="align-middle"><a type="submit" class="text-black text-decoration-none">Cancella</a></td>
                                    </form>
                                </tr>
                            @endif
                        @endforeach
